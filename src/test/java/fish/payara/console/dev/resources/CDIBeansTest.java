...
@RunAsClient
public class CDIBeansTest extends ConsoleDeployment {

    @Test
    public void beans_schema_is_valid() {
        JsonArray beans = getJsonArray("cdi/beans");

        if (!beans.isEmpty()) {
            JsonObject bean = beans.getJsonObject(0);
            assertTrue(bean.containsKey("beanClass"));
        }
    }

    @Test
    public void scoped_beans_schema_is_valid() {
        JsonArray beans = getJsonArray("cdi/scoped-beans");
    ...
    public void injection_points_schema_is_valid() {
        JsonArray ips = getJsonArray("cdi/injection-points");
